...
async function run() {
    ...
    if (destinations.length > 0) {
        ...
        r = await request('GET', `/api/destinations/details/${destinationId}`, null, true);
        log('GET  /api/destinations/details/:id', r, 200);
    } else {
    ...
    log('GET  /api/game/leaderboard (public → 200)', r, 200);

    if (adminId) {
        r = await request('GET', `/api/game/users/${adminId}/profile`, null, false);
        log('GET  /api/game/users/:id/profile', r, 200);
    }

    r = await request('POST', '/api/game/user/update-frame', { frameSlug: 'default-frame' }, true);
    log('POST /api/game/user/update-frame', r, 200);
